...
module.exports = class CampusApi {
  ...
  getCampusListNames() {
    this.campusListNames = this.conf.software.map((item) => {
  ...
  async getToken() {
    let api = new Query(
      this.conf.connectConfig,
      null,
      this.conf.connectConfig.data
    );
  ...
  async getOneList(listName) {
    ...
    this.conf.queryConfig.get.options.path =
      this.conf.queryConfig.get.options.pathStem + 'dulb-patron' + listName;
  ...
  async convertEmailToUniq(email) {
    ...
    this.conf.queryConfig.convert.options.path =
      this.conf.queryConfig.convert.options.pathStem + escapedEmail;
